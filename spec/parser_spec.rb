...
describe Cinema::Parser, :vcr do
  ...
  let(:parser) {described_class.new(collection)}
  ...
  let(:yml_path) { Pathname.new('data.yml') }
  let(:html_path) { Pathname.new('movies.html') }
  ...
  context 'save_to_yml' do
    ...
    it { expect(yml).to have_attributes(count: 250) }
  end

  context 'save_to_html' do
    it { expect(html_path).to exist }
    it { expect(html.children.length).to eq(129) }
  end
