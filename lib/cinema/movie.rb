module Cinema
  # Movie class contains info about cirtain movie
  class Movie
    ...
    def self.create(row, collection)
      _, period_settings = PERIODS.detect do |_period, value|
        value[:years] === row['year']
      end
      if period_settings.nil?
        raise 'У фильма неподходящий год. В базе могут быть только фильмы, снятые с 1900 года по настоящий.'
      end
      movie_class = period_settings[:movie_class]
      movie_class.new(row['link'], row['title'], row['year'], row['country'], row['date'], row['genre'], row['length'], row['rating'], row['director'], row['actors'], collection)
    end

    def has_genre?(genre)
      unless @collection.genre_exists?(genre)
        raise ArgumentError, 'Аргумент задан с ошибкой, либо такого
         жанра не существует.'
      end
      @genre.include? genre
    end
